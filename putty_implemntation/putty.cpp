...
void orientate_LR() {
  ...
  do {
    buffer_left = long_ir_dist(IR_LeftPIN_Back);
    buffer_right = long_ir_dist(IR_RightPIN_Back);
    SerialCom->print("Left IR: ");
    SerialCom->println(ir_left);
    SerialCom->print("Right IR: ");
    SerialCom->println(ir_right);
...
// const int ORIENTATE_STABLE_COUNT = 2; // Required consecutive readings within tolerance (from second code)
// const float ORIENTATE_KP = 7.0;       // Proportional gain (from second code)
// const int LOOP_TIME_MS = 25;          // Loop delay time
...
//         buffer_left = long_ir_dist(IR_LeftPIN_Back);
//         buffer_right = long_ir_dist(IR_RightPIN_Back);
//         buffer_right =buffer_right -3;

//         // Update main variables only if readings are valid ( > 0 )
//         // This uses the "last known good value" approach
//         if (buffer_left > 0) {
//             ir_left = buffer_left;
//         }
//         if (buffer_right > 0) {
...
//         // --- Start of Core Logic Block ---
//         // Perform alignment logic only if BOTH sensors have provided at least one valid reading
//         if (ir_left > 0 && ir_right > 0) {
//             // Calculate error using last known good values
//             error = ir_left - ir_right;

//             // **MODIFICATION: Stop motors before evaluating the error for this cycle**
//             stop();

//             // Check if error is within tolerance
//             if (abs(error) <= tolerance) {
//                 // Within tolerance - increment stability counter
//                 stable_count++;
//                 // Motors are already stopped from above
//                 // Optional: Print stability increase immediately for debugging
//                 // SerialCom->print("  Stable count increased: "); SerialCom->println(stable_count);
//             } else {
//                 // Out of tolerance - reset stability and rotate
...
//                 // Set the global speed variable used by cw()/ccw()
//                 speed_val = rotation_speed;

//                 // Rotate based on the sign of the error
//                 if (error > 0) { // Left is further away, turn counter-clockwise (left)
//                     ccw(); // Start CCW rotation
//                 } else { // Right is further away (or equal, error <=0), turn clockwise (right)
//                     cw();  // Start CW rotation
//                 }
//             }
//         } else {
...
//         // Print sensor readings and status periodically
...
//             // Only print error etc. if we have valid readings for both
//             if (ir_left > 0 && ir_right > 0) {
//                  // Error is already calculated within the logic block
//                  SerialCom->print(" | Err:"); SerialCom->print(error, 1);
//                  SerialCom->print(" | Stable:"); SerialCom->print(stable_count);
//                  // Indicate current action based on state
//                  if(stable_count > 0 && abs(error) <= tolerance) {
//                     SerialCom->print(" | Action: Stable/Stopped");
//                  } else if (abs(error) > tolerance) {
//                     SerialCom->print(" | Action: Rotating");
//                  } else {
//                     SerialCom->print(" | Action: Stopped (Error?)");
//                  }
...
//         // Main loop delay
//         delay(LOOP_TIME_MS); // 25ms delay

//     } while (stable_count < required_count);

//     // Orientation complete or loop exited unexpectedly
...
void move(float y_dist, float x_dist, float target_angle, int y_direction, int servo_direction) {
  ...
  do {
    ...
    update_cords(current_x, current_y);
