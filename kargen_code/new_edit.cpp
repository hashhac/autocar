...
STATE test() {
  ...
  //Corner Check
  // SerialCom->println("Sweeping");
  // Sweep(sweep_dist);
  // corner_angle = detectCorner(sweep_dist);

  // delay(2000);
  // myservo.write(90);


  // //rotate/drive if needed away from corner
  // if(corner_angle != -1) {
  //  rotate(corner_angle);
  // }
  // delay(2000);
  ...
  //Orientate parallel with first wall
  SerialCom->println("Orientation");
  orientate();
  delay(2000);


  //Rotate 90
  SerialCom->println("Rotation 1");
  rotate(rotate_angle);
  delay(2000);

  // // DriveParallel to the next wall
  SerialCom->println("Wall Follow");
  // // wallFollow();
  // forwardUntil(driving_until_dist, kp);
  myservo.write(0);
  move(20,20,0);
...
//   if ((found_angle <= 120) && (found_angle >= 90)) {
//     return 15;
//   } else if ((found_angle <= 90) && (found_angle >= 60)) {
//     return -15;
//   }
// }
...
void orientate() {
  ...
  do {
    ...
    //   SerialCom->print("Left IR Sensor: ");
    // SerialCom->println(ir_left);
    // SerialCom->print("Right IR Sensor: ");
    // SerialCom->println(ir_right);

    if (abs(ir_left - ir_right) < tolerance) {
...
void recalibrateGyro() {
  ...
  delay(2000);  // Allow time for the robot to stabilize
...
void updateCurrentAngle() {
  ...
  // Wrap the current angle within 0-359 degrees
...
void move(float y_dist, float x_dist, float target_angle) {
  ...
  do {
    ...
    // Error Values
    y_err = -1*(y_dist/abs(y_dist))*(abs(y_dist) - current_y);
